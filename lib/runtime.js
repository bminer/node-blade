...
(function() {
	...
	*/
	runtime.loadTemplate = function(baseDir, filename, compileOptions, cb) {
		...
		{
			if(cachedViews[baseDir + "/" + filename])
				return cb(null, cachedViews[baseDir + "/" + filename]);
	...
	runtime.include = function(baseDir, filename, info, cb) {
		...
		info.inc = true;
		runtime.loadTemplate(baseDir, filename, runtime.compileOptions, function(err, tmpl) {
			if(err) return cb(err);
