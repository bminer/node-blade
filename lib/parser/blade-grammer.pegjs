...
TODO: Commenting out Blade code doesn't work so well (maybe allow "//|"
to implement short-hand comment block, followed by text block?)
...
attribute_code =
		code:[^, \t\r\n\(\)\"]+ str:quoted_string?
		{return code.join("") + (str == "" ? "" : JSON.stringify(str) );}
	/
		str:quoted_string code:attribute_code?
		{return JSON.stringify(str) + code;}
