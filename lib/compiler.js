...
Compiler.prototype._compileNode = function(node) {
	...
	switch(node.type)
	{
		...
		case 'code':
			...
			if(node.multiline)
			{
				var e = new Error("Code statements spanning mulitple lines " +
					"must be indented underneath a line of code.");
				e.line = node.line, e.column = node.col;
				throw e;
			}
			...
			{
				//Process mutli-line code statements first
				for(var i = 0; i < node.children.length; i++)
				{
					if(node.children[i].type == "code" && node.children[i].multiline)
					{
						this._pushOff(node.children[i].code + "\n");
						if(node.children[i].children.length > 0)
						{
							var e = new Error("Code statements spanning mulitple lines " +
								"cannot have child nodes.\nPlease check to ensure " +
								"nothing is indented underneath the `_` line of code.");
							e.line = node.children[i].line, e.column = node.children[i].col;
							throw e;
						}
					}
					else
				...
				//Now process all other child nodes
				var insertCurlyBrace = i < node.children.length;
				if(insertCurlyBrace)
					this._pushOff("{");
				for(; i < node.children.length; i++)
					this._compileNode(node.children[i]);
				//Now we end the code statement with a curly brace or semicolon
				if(insertCurlyBrace)
					this._pushOff("}");
				else
					this._pushOff(";");
