...
function Compiler(string, opts) {
	this.string = string;
	this.options = opts || {};
	//Use options.doctypes or copy default doctypes
	if(this.options.doctypes)
		this.doctypes = this.options.doctypes;
	else
	{
		this.doctypes = [];
		for(var i in doctypes)
			this.doctypes[i] = doctypes[i];
	}
	//Use options.filters or copy default filters
	...
	//Use options.inlineTags or copy default inlineTags
	if(this.options.inlineTags)
		this.inlineTags = this.options.inlineTags;
	else
	{
		this.inlineTags = [];
		for(var i in inlineTags)
			this.inlineTags[i] = inlineTags[i];
	}
...
Compiler.prototype.compile = function(cb) {
	...
	try {
		...
		var template = 'runtime = runtime || blade.runtime; ' +
			'var buf = [], __ = {buf: buf, blocks: {}, locals: locals || {}};';
		...
		for(var i = 0; i < this.includes; i++)
			if(this.options.minify !== true)
				template += "} catch(e){runtime.rethrow(e, __);} });";
			else
				template += "});";
	...
	switch(node.type)
	{
		case 'tag':
			...
			if(node.classes.length > 0)
			{
				if(attrs['class'] == undefined)
					...
					attrs['class'].text += (attrs['class'].text.length > 0 ? " " : "") + node.classes.join(" ");
				else
		...
		case 'text':
			...
			if(this.lastNode && this.lastNode.type == "text")
				node.text = "\n" + node.text;
			//TODO: Possibly implement some sort of interpolation here
			if(node.escape)
				str += "buf.push(" + JSON.stringify(runtime.escape(node.text) ) + ");";
			else
				str += "buf.push(" + JSON.stringify(node.text) + ");";
			break;
		case 'code':
			//Code does not output into the view and may have children
		...
		case 'include':
			...
			this.includes++;
			str += "runtime.include(" + JSON.stringify(
				this.basedir + "/" + node.filename) +
				", __, function(err) {";
			if(this.options.minify !== true)
				str += "try{";
