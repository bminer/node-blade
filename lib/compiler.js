...
Compiler.prototype.compile = function(cb) {
	...
	try {
		...
		if(this.options.minify !== true)
			this._pushOff(ns + ".filename = " + JSON.stringify(this.filename) + ";\n\ntry {");
...
Compiler.prototype._push = function(str) {
	...
	{
		this.buf.push(this.templateNamespace + ".push(" + str);
		this.inPush = true;
	}
};
...
Compiler.prototype._compileNode = function(node) {
	...
	switch(node.type)
	{
		...
		case 'include':
			...
			this.includes++;
			if(node.filename)
				this._pushOff(ns + ".r.include(" + JSON.stringify(this.options.basedir) +
		...
		case 'block':
			...
			this._pushOff(ns + ".r.blockDef(" + JSON.stringify(node.name) + ", __, function(__" +
				paramStr + ") {");
			for(var i in node.children)
				this._compileNode(node.children[i]);
			this._pushOff("});");
		...
		case 'render':
			if(node.arguments.length > 0)
				this._pushOff(ns + ".r.blockRender(" + JSON.stringify(node.name) + ", __, " +
					node.arguments + ");");
			else
				this._pushOff(ns + ".r.blockRender(" + JSON.stringify(node.name) + ", __);");
			break;
