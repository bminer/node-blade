	...
	switch(node.type)
	{
		case 'tag':
			...
			for(var i in attrs)
			{
				...
				{
					if(attrs[i].escape)
						str += "buf.push(' " + i + "=" + bladeutil.quote(
			...
			if(varAttrs.length > 0)
				str += "runtime.attrs({" + varAttrs.substr(1) + "}, buf, this);";
		...
		case 'include':
			...
			if(node.filename)
				str += "runtime.include(" +
					JSON.stringify(this.options.basedir + "/" + node.filename) +
					", __, function(err) {";
			else
				str += "runtime.include(" + JSON.stringify(this.options.basedir + "/") +
		...
		case 'call':
			/*
				(node.id != null || node.classes.length > 0 ?
					"runtime.root(" + JSON.stringify(node.id) + ",'" +
					node.classes.join(" ") + "'," : "(") +
			*/
			var thisObj = {};
			if(node.id != null)
				thisObj.id = node.id;
			if(node.classes.length > 0)
				thisObj.classes = node.classes;
			if(node.output)
				str += node.output.to + (node.output.append ? "+" : "") + "=" +
					"__.func[" + JSON.stringify(node.name) + "].call(" +
					JSON.stringify(thisObj) + "," + node.arguments + ");"
			else
				str += "buf.push(__.func[" + JSON.stringify(node.name) + "].call(" +
					JSON.stringify(thisObj) + "," + node.arguments + ") );"
			break;
		default:
