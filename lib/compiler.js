...
Compiler.prototype._compileNode = function(node) {
	...
	switch(node.type)
	{
			...
			for(var i in attrs)
			{
				...
				if(attrs[i].text)
				{
					...
					varAttrs += "," + i + ":{a:" + JSON.stringify(attrs[i].append) +
						",v:" + attrs[i].code + ", e:" + attrs[i].escape + "}";
				else
					varAttrs += "," + i + ":{v:" + attrs[i].code + ", e:" +
						attrs[i].escape + "}";
			}
		...
		case 'text':
			...
			if(node.escape)
				this._push(ns + ".r.escape(" + bladeutil.interpolate(
					JSON.stringify(node.text), ns) + ")");
			else
				this._push(bladeutil.interpolate(JSON.stringify(node.text), ns) );
		...
		case 'code':
			...
			this._pushOff(node.code.replace(/\/\/[^\n]*/g, "") + //Get rid of "//" comments
